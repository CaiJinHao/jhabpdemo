...
import OperationModalCategory from './components/OperationModal';
const CategoryList = () => {
  ...
  const [currentOperation, setCurrentOperation] = useState<API.Demo.CategoryDto | undefined>(
    undefined,
  );
  ...
  const handlerIsDeleted = async (record: API.Demo.CategoryDto, action: any) => {
    if (record.isDeleted) {
      confirm({
  ...
  const loadDetail = async (operation: ViewOperator, record: API.Demo.CategoryDto) => {
    setDetailOperation(operation);
    setVisibleOperation(true);
    const detailDto = await defaultService.Get(record.id);
    setCurrentOperation(detailDto);
  };
  const edit = async (record: API.Demo.CategoryDto) => {
    await loadDetail(ViewOperator.Edit, record);
  };
  const detail = async (record: API.Demo.CategoryDto) => {
  ...
  const columns: ProColumns<API.Demo.CategoryDto>[] = [
    {
      title: intl.formatMessage({ id: 'DisplayName:Category:Name', defaultMessage: '' }),
      dataIndex: 'name',
    },
    {
      title: intl.formatMessage({ id: 'DisplayName:Category:Description', defaultMessage: '' }),
    ...
    },
    {
      ...
      render: (text, record, index, action) => {
        return (
          <Switch
            disabled={!(access['MyDemo.Categorys.Recover'] && access['MyDemo.Categorys.Delete'])}
    ...
    },
    {
      ...
      render: (_, record) =>
        !record.isDeleted && [
          access['MyDemo.Categorys.Update'] && (
